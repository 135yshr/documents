...
Martin Fowler のテストピラミッドでは、ユニットテストを最も多く、インテグレーションテスト、E2Eテストの順に減らすことが推奨されています（参考：[TestPyramid](https://martinfowler.com/bliki/TestPyramid.html)）。しかしDDDのレイヤー構造にこのピラミッドをどう対応させるかは、自明ではありません。

この記事では、DDDの各レイヤーに適したテスト戦略を、Go の具体的なコード例とともに共有します。
...
最も投資対効果が高いのは**ドメイン層のユニットテスト**です。ドメイン層は外部依存がなく、テストが高速で安定しているため、テストピラミッドの土台として最適です。
...
値オブジェクトは生成時の制約（バリデーション）と等値性をテストします。外部依存がないため、最もテストしやすい要素です。
...
集約の状態遷移が複雑な場合、テーブル駆動テストで全パターンを網羅します。
...
- **ドメイン層の内部（値オブジェクト、エンティティ）**: モックしない。実際のオブジェクトを使います
...
// discountRuleFinder はテスト用のモックです。
...
Go のテストでは、モックライブラリ（gomock, mockery 等）を使わず、テストファイル内に小さなスタブ構造体を定義するスタイルが広く使われています。理由は次のとおりです。
...
ユースケース層のテストは、ドメインオブジェクトとインフラストラクチャの**連携**を検証するインテグレーションテストです。リポジトリはモックし、ドメインオブジェクトは実物を使います。
...
func TestConfirmOrderUseCase(t *testing.T) {
    t.Run("注文を確定してイベントを発行する", func(t *testing.T) {
        ...
        bus := &spyEventBus{}
        order, _ := model.NewOrder("order-1", "customer-1", []model.OrderItem{
...
| 種類             | 用途                         | 例                  |
| ---------------- | ---------------------------- | ------------------- |
| スタブ（Stub）   | 固定値を返す                 | `inMemoryOrderRepo` |
| スパイ（Spy）    | 呼び出しを記録する           | `spyEventBus`       |
| フェイク（Fake） | 簡易実装を提供する           | インメモリDB        |
| モック（Mock）   | 期待される呼び出しを検証する | gomock生成のモック  |

DDD のテストでは、スタブとスパイで十分なケースがほとんどです。モックの検証（「このメソッドが何回呼ばれたか」）は実装の詳細に依存するため、過度に使うとリファクタリング耐性が下がります。
...
func TestOrderRepository_SaveAndFindByID(t *testing.T) {
    ...
    order, _ := model.NewOrder("test-order-1", "customer-1", []model.OrderItem{
...
Go の標準的なテスト命名に従います。`Test{関数名}_{シナリオ}` の形式を使い、日本語で具体的なシナリオを記述します。
...
func newTestOrder(t *testing.T) *model.Order {
    ...
    order, err := model.NewOrder("order-1", "customer-1", items)
    if err != nil {
        t.Fatalf("テスト用注文の生成に失敗しました: %v", err)
    }
    return order
...
### 4. テストピラミッドのバランスを意識する

```text
        /  E2E  \           → 少数（APIの正常系のみ）
       /  統合   \          → 中程度（ユースケース + リポジトリ）
      / ユニット  \         → 多数（集約・値オブジェクト・ドメインサービス）
```

ドメイン層のユニットテストが最も多く、最も頻繁に実行されるようにします。インテグレーションテストはCI環境で実行し、E2Eテストは主要なフローに限定します。
...
- **Go ではスタブとスパイで十分**なケースがほとんどです。モックライブラリの導入は、interfaceが大きい場合に検討します
...
| 内容 | 出典 |
| --- | --- |
| テストピラミッド | Martin Fowler, [TestPyramid](https://martinfowler.com/bliki/TestPyramid.html) |
