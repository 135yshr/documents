...
分析処理で、同じ会話を二重に分析しないためのチェックが必要になりました。冪等性チェックは「ドメインの関心事ではないか」という議論もあります。しかしここでは「既存の分析結果があればそれを返す」という**アプリケーション固有のフロー制御**であり、Interactor層が適切だと判断しました。
...
ただ、私のプロジェクトでは**複数機能に同時にInteractor層を追加する場面**がありました。認可チェックを全機能に入れるとき、Interactor層がある機能とない機能が混在していると問題になります。修正箇所の特定と影響範囲の把握に時間がかかるからです。最初から統一しておく方がこうした横断的変更のコストが低いと感じました。
