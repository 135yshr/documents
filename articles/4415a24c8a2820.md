...
戦術的DDDは「1つの[境界づけられたコンテキスト](https://zenn.dev/135yshr/articles/d27112edbad1fc)の内部をどう設計するか」に焦点を当てます。

境界づけられたコンテキスト（Bounded Context）とは、特定のドメインモデルが一貫した意味を持つ範囲のことです。たとえば「注文」コンテキストでの「商品」と「カタログ」コンテキストでの「商品」は、同じ言葉でも含む情報や振る舞いが異なります。この境界を明確にすることで、各チームは自分のコンテキスト内のモデルを自由に進化させられます。

しかし現実のシステムでは、複数のコンテキストが連携して動いています。
...
**コンテキスト（Context）** : 本記事で繰り返し登場する「コンテキスト」とは、ある言葉や概念が特定の意味を持つ **文脈・前提の範囲** のことです。日常会話でも「文脈によって意味が変わる」と言いますが、DDDではこの「文脈の範囲」をソフトウェア設計の単位として明確に区切ります。たとえば「商品」という言葉は、注文の文脈では価格や数量が重要ですが、カタログの文脈では説明文や画像が中心です。この文脈の違いを「境界づけられたコンテキスト（Bounded Context）」として定義し、それぞれの範囲内でモデルの一貫性を保ちます。
