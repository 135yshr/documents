...
Go では構造体は値コピーされますが、スライスやマップは内部で参照を共有するため注意が必要です。本記事のコードでは `toEventItems` で新しいスライスを生成しているため、呼び出し元がスライスを変更してもイベントに影響しません。もし `toEventItems` を使わず既存のスライスを直接イベントへ埋め込む場合は、明示的なコピーが必要です。
...
func (p *OrderProjection) Handle(e event.Event) error {
    ...
    case event.OrderCancelled:
        ...
        return err
    default:
        return fmt.Errorf("未対応のイベントタイプ: %s", e.EventType())
    }
}
