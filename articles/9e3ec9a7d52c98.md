...
CQRSはBertrand Meyerが提唱した**CQS（Command Query Separation）原則**をアーキテクチャレベルに拡張したものです。CQSは個々のメソッドを「値を返す（Query）」と「状態を変える（Command）」に分ける原則です。CQRSはこの考え方をモデル全体の設計に適用します。
