...
remote: 
remote: Create a pull request for 'refine/error-handling-article' on GitHub by visiting:        
remote:      https://github.com/135yshr/documents/pull/new/refine/error-handling-article        
remote: 
To github.com:135yshr/documents.git
 * [new branch]      refine/error-handling-article -> refine/error-handling-article
branch 'refine/error-handling-article' set up to track 'origin/refine/error-handling-article'.</bash-stdout><...

### Prompt 16

教科書的には「インフラ層はインフラエラーをそのまま返し、アプリケーション層で解釈すべき」とされています。しかし実務ではインフラ層でDBエラーをドメインエラーに変換するパターンが有効です。

とありますが、「有効でした」の方が伝わりやすのではないでしょうか？

### Prompt 17

gorm.ErrRecordNotFoundをmodel.ErrTaskNotFoundに変換しています。これを「アンチパターン」と呼ぶ記事もありますが、私は境界での翻訳として合理的だと考えています。

とありますが、この実装がアンチパターンではないことを示す情報はありますか？
