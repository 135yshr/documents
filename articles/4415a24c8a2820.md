...
コンテキストマップは単なるアーキテクチャ図ではなく、**チーム間の力関係や連携方針**も表現します。
...
**コンテキスト（Context）** : 本記事で繰り返し登場する「コンテキスト」とは、ある言葉や概念が特定の意味を持つ **文脈・前提の範囲** のことです。日常会話でも「文脈によって意味が変わる」と言いますが、DDDではこの「文脈の範囲」を設計の単位として明確に区切り、「境界づけられたコンテキスト（Bounded Context）」と呼びます。詳しくは本文の「戦術的DDDだけでは解決しない問題」セクションを参照してください。
