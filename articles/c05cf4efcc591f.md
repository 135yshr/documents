...
この原則に従い、ドメインイベントを使って集約間の通信を**分離**します。後述するインプロセスバスを使うとハンドラは別 goroutine で処理されますが、ハンドラの完了は呼び出し元に通知されません（非同期・ファイア＆フォーゲット）。信頼性が必要な場合は Outbox パターンや外部メッセージキューと組み合わせてください。
...
if err := bus.Subscribe("OrderConfirmed", func(ctx context.Context, e event.Event) error {
