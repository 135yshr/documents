...
Command系はドメイン層を経由するため4層すべてが関わりますが、Query系はドメインモデルを経由しないため、Application層とInfrastructure層の2層で完結します。どちらの経路でも**Infrastructure層が上位層のインターフェースを実装する（依存性逆転）** という原則は共通です。
