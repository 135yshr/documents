...
エンティティ、値オブジェクト、リポジトリ、ユースケース…戦術的なDDDパターンを導入してコードの品質は上がりました。しかし、システムが複数のチームやサービスにまたがると、別の問題が出てきます。
...
Eric Evansは、こうした問題への対処として**コンテキストマップ**を提唱しました。

> A Context Map is a tool for defining the relationships between bounded contexts. It is not just a diagram—it is a living document that captures the organizational and integration patterns.
>
...
graph LR
    ...
    Order -- "顧客-供給者<br/>(下流)" --> Inventory
    Inventory -- "顧客-供給者<br/>(上流)" --> Order
    Order -- "腐敗防止層" --> Payment
    Order -- "公開ホストサービス<br/>+ イベント" --> Shipping
    Order -- "イベント" --> Notification
    Shipping -- "順応者" --> Notification
```

このマップから、以下のことが読み取れます。

- 注文と決済サービスの間にはACLが必要です（外部サービスのモデルから自分を守るため）
- 注文と在庫は顧客-供給者の関係です（在庫チームが注文チームのニーズに応じてAPIを調整します）
...
> Shared Kernel is a relationship where two teams share a small but common model. Designate some subset of the domain model that the two teams agree to share. Keep the kernel small.
>
...
type stockChecker interface {
    CheckAvailability(ctx context.Context, productID string, quantity int) (*inventory.Availability, error)
}
...
下流が上流のモデルにそのまま従うパターンです。上流チームが下流の要求に応じる余裕がない場合や、上流のモデルが十分に整っている場合に採用します。
