...
見出し、リスト、コードブロック、テーブルなどをターミナル向けに整形して表示します。通常は長い文書をページャに渡します。
...
ただし、Mermaidをインライン画像として実際に表示した場合はページャを使いません。kittyやiTerm2向けの画像エスケープシーケンスを `less` に通せないためです。ページャを使いたい場合は、`-mermaid-render ascii` または `-mermaid-render source` を指定します。
...
`auto` では、まず実行環境から文書全体の表示方針を決めます。TTYかどうか、`NO_COLOR`、`TERM`、`TERM_PROGRAM` などを見て画像プロトコルを使えるか判断し、使えなければその実行ではテキストアートを使います。

画像表示を選んだ場合でも、個々の図でMermaid CLIが見つからない、ラスタライズに失敗する、といった場合はテキストアートへフォールバックします。さらに、テキストアートに対応していない図種はMermaidソースを表示します。

```mermaid
flowchart TD
    A[auto] --> B{画像プロトコルを利用できるか}
    B -- No --> C[文書全体をText Artで表示]
    B -- Yes --> D[Imageを選択]
    D --> E{画像化に成功したか}
    E -- Yes --> F[Image]
    E -- No --> G[Text Art]
    ...
    G --> H
    H -- Yes --> I[Text Art]
    H -- No --> J[Mermaid Source]
```

また、`-width` が狭く、Mermaid用のプレースホルダーを保持できない場合は文書全体をソース表示に切り替えます。
...
最終的には、環境や図の種類に応じて `image`、`ascii`、`source` のいずれかで表示します。
...
そこでmd2pdfにConsole出力を追加し、Mermaidは環境や図の種類に応じて `image`、`ascii`、`source` のいずれかで表示するようにしました。

画像を表示できるなら画像で出す。無理ならテキストアートにする。テキストアートに対応していない図なら、元のMermaidを表示する。
