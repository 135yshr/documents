...
この設計のポイントは、`raise`（コマンド実行時）と `apply`（リプレイ時）を分離していることです。`apply` は「すでに検証済みのイベント」を前提としており、外部から任意に呼び出されることは想定していません。整合性の保証はコマンドメソッド（`NewOrder`, `Confirm` 等）側で担保します。リプレイ時にはバリデーションを通さず、`apply` だけを呼び出します。
