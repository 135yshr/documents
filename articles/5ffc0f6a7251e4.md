...
本記事はDDD×クリーンアーキテクチャ連載の一部です。イベントソーシングの基本概念からGoでの実装パターンまでを解説します。根拠となる一次情報源は、末尾の参考文献に記載しています。
...
- `infrastructure/postgres` — インフラ実装
- `usecase` — ユースケース
...
この記事では、Go でイベントソーシングを実装するために必要な概念と、実装パターンを具体的なコードで解説します。コード例は説明の都合上セクションごとに分割して掲載していますが、同一ファイルのコードは結合してご利用ください。
...
上記の表で「イベントのアップキャストで対応」と書きましたが、補足します。アップキャストとは、古いバージョンのイベントを現在のスキーマに変換する処理です。たとえば、`OrderCreated` イベントに後から `currency` フィールドを追加した場合、デシリアライズ時にデフォルト値 `"JPY"` を補完する変換処理を挟みます。Go では `deserializeEvent` 関数内で JSON を `map[string]any` にパースしてバージョンを判定し、不足フィールドを補完してから構造体にマッピングする方法が実用的です。ただし、イベントのバージョニング戦略は本記事の範囲を超えるため、詳細は別記事で扱います。
...
func NewOrder(id, customerID string, items []orderItem, now time.Time) (*Order, error) {
    if len(items) == 0 {
...
また、Go ではイベントを値型（構造体の値）として `raise` に渡しています。`int` や `string` などの値型フィールドはコピーで保護されます。ただし、スライスやマップは参照が共有されるため、値コピーだけではイベントの不変性を完全には保証できません。本記事のコードでは `toEventItems` でスライスを新規に生成しているため安全ですが、既存のスライスをそのまま埋め込む場合は注意が必要です。
...
`Save` メソッドの `defer tx.Rollback()` は、コミット成功後に呼ばれた場合は何もしません。`database/sql` の仕様で、コミット済みトランザクションの Rollback は無視されます。
...
イベントストアからイベントを読み出し、`apply` を順に呼び出すことで集約を復元します。復元には2つのパターンがあります。`ReplayOrder` はイベント列からゼロベースで集約を構築する関数で、スナップショットがない場合に使います。一方、前述の `ReplayFrom` メソッドは既存の集約（スナップショットから復元したもの）に対して追加のイベントを適用します。
...
// スキーマ変更時は破棄してイベントから再構築できるため、互換性の問題は生じません。
...
type SnapshotStore interface {
    ...
    // Load はスナップショットを取得します。
    // スナップショットが存在しない場合は (nil, nil) を返します。
...
func (uc *OrderUseCase) LoadOrder(id string) (*model.Order, error) {
    ...
    events, err := uc.eventStore.LoadFrom(id, fromVersion)
    if err != nil {
        return nil, err
    }
    ...
    if order.Version()-fromVersion >= snapshotInterval {
        ...
        } else if err := uc.snapshotStore.Save(&repository.Snapshot{
            AggregateID: id,
...
イベントソーシングは CQRS なしでも使えます（クエリのたびにリプレイすればよい）。しかし、一覧表示や検索のようなクエリ操作でリプレイを行うのは非効率です。そのため、実用上は**CQRS（Command Query Responsibility Segregation）**を組み合わせることが多いです。
...
プロジェクションは、イベントを購読して読み取りモデルを更新するハンドラです。以下の `Handle` メソッドは、イベント配信メカニズムから呼び出される想定です。配信方法としてはイベントストアへのポーリング、メッセージキュー（Kafka、NATS 等）経由の Pub/Sub、PostgreSQL の `LISTEN/NOTIFY` などが考えられます。本記事では配信基盤の実装は扱いませんが、プロジェクションは配信方法に依存しない設計です。
