...
graph LR
    A[ドメイン層] --> B2
    subgraph ACL
        B2[Translator<br/>変換を担当] --> B1[Adapter<br/>通信を担当]
    end
    B1 --> C[外部システム]
```
...
Adapterは外部システムとの通信プロトコルを担当します。ユースケース層が定義するインターフェースを実装し、内部でTranslatorを使います。
...
ACLを過剰に導入すると、翻訳層のメンテナンスコストが増えます。外部のモデルとドメインモデルが近い場合は、Translator（変換層）を設けずに外部APIのクライアントをそのまま薄くラップするだけで十分です。
