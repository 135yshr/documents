...
これは私も導入当初に悩んだ点です。YAGNIは「必要になるまで作らない」原則ですが、Interactor層は**アーキテクチャの一貫性**を提供する側面があります。チームメンバーが「この機能のロジックはどこにあるか」と迷わない統一構造は、パススルーのコストに見合う価値がありました。

また、私の経験では**パススルーのまま残ったInteractorはほとんどありません**でした。プロジェクトの成長に伴い、キャッシュ、認可チェック、監査ログ、メトリクス収集など、何らかのロジックが追加されていきました。
